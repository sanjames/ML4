...
%size(X) % 5000 X 400
%%%%%%%%%San %%%%%%%%%%
%a1 = [ones(m, 1) X]; % a1 = X adding column with ones resulting in a matrix of size 5000 X 401
%z2 = a1 * Theta1'; %size of Theta1' is 401 X 25, a1 is 5000 X401 => z2 is 5000 X 25
%a2 = [ones(m,1) sigmoid(z2)]; % size of a2 is 5000 X 26

%z3 = a2 * Theta2'; %size of a2 is 5000 X 26 Theta2' is 26 X 10. Hence we need to add 1 column ones to a2 
%resulting in => z3 is 5000 X 10
...
hTheta = sigmoid(Theta2 * a2'); % h_theta equals z3

loghTheta = log(hTheta); %10 X 5000
minloghTheta = log(1-hTheta); % 10 X 5000 y is 5000 X 1
...
J = (-1/m) * sum(sum(yk .* loghTheta + (1-yk) .* minloghTheta));  
